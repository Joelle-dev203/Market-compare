...
class ProductSearchController extends Controller
{
    public function search(Request $request)
    {
        ...
        $isTopDeals = $request->has('deal') && $request->deal == 'true';
        ...
        // Apply Top Deals filter using pivot table columns
        if ($isTopDeals) {
            $productsQuery->whereHas('vendors', function($query) {
                $query->whereNotNull('old_price')
                      ->whereColumn('product_vendor.price', '<', 'product_vendor.old_price');
            });
        }
        ...
        // Randomize only if no search query, category, location, or top deals filter is active
        if (empty($searchTerm) && empty($categoryFilter) && empty($locationFilter) && !$isTopDeals) {
        ...
        return view('search', compact(
            'products', 'trips', 'searchTerm', 'categoryFilter', 'locationFilter', 'availableLocations', 'productsCount', 'vendorCount', 'isTopDeals'
        ));
    }
}